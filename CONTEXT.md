...
Animated navigations between pages in a dastro/Astro site. This package enables them and supplies the shared *chrome* and presets.
...
**Named group**:
A `view-transition-name` on chrome that always participates. Projects attach it with the existing classes `.view-transition-header` (follows the **Preset**) and `.view-transition-site-header` (persist).
...
*Avoid*: layout, shell (unless you mean these two)
...
- **Chrome** always uses the same **Named groups**; the page header follows the **Preset**, the site-header always persists
