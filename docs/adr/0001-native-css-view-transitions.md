...
ClientRouter is a different product (SPA DOM swap, `transition:persist`, `astro:page-load`, scripts that do not re-run). The visual CSS (`::view-transition-*`, named chrome) can look the same, but swapping the router later rewrites the package. Native CSS is the one most dastro sites already have.
